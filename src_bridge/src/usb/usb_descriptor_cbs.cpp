...
//     if(usb_string_desc != nullptr){
//         usb_string_desc->clear();
//     }

// std::vector<uint8_t>

    // usb_string_desc = std::make_shared<std::vector<uint8_t>>(g_usb_string_descriptor->generateDescriptorBlock());

    // return (uint16_t*)config_qualifier_desc->data();
}  
