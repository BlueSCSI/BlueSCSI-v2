...
uint8_t BlueScsiBridge::initiator_id;
uint8_t BlueScsiBridge::configured_retry_count;
...
void BlueScsiBridge::init()
{
    ...
    while (!initialization_complete)
    {
        ...
        for (int target_id = 0; target_id < 8; target_id++)
        {
            ...
            startstopok &= StartStopUnit(target_id, 0, true, false);
            if (!startstopok)
            {
            ...
            std::shared_ptr<USB::DiskInfo> cur_target = std::make_shared<USB::DiskInfo>();
            ...
            USB::DiskInfoList.push_back(cur_target);

        } // end for each target ID

        if (USB::DiskInfoList.size() < 1)
        {
            log("SCSI bus scan failed. Adding RAM Disk");
            std::shared_ptr<USB::DiskInfo> cur_target = std::make_shared<USB::DiskInfo>();
            cur_target->target_id = USB::DiskInfo::RAM_DISK;
            cur_target->ram_disk_buffer = (uint8_t *)&MscExampleDisk::msc_disk0;
            cur_target->sectorcount = MscExampleDisk::DISK_BLOCK_NUM;
            cur_target->sectorsize = MscExampleDisk::DISK_BLOCK_SIZE;
            USB::DiskInfoList.push_back(cur_target);
        }

        if (USB::DiskInfoList.size() > 0)
        {
...
//                             uint8_t *buffer, uint8_t buffersize)
    ...
    shared_ptr<USB::DiskInfo> mydisk = USB::DiskInfoList[0];
    uint8_t scsi_buffer[128];
    bool scsires = Read10(mydisk->target_id, 0, 1, mydisk->sectorsize, scsi_buffer, sizeof(scsi_buffer));
    for(int i=0; i<sizeof(scsi_buffer); i++)
    {
    ...
    // Add MSC Disk 0 for a known-good example
    // std::shared_ptr<USB::DiskInfo> cur_target = std::make_shared<USB::DiskInfo>();
    // cur_target->target_id = USB::DiskInfo::RAM_DISK;
    // cur_target->ram_disk_buffer = (uint8_t*)MscExampleDisk::msc_disk0;
    // cur_target->sectorcount = MscExampleDisk::DISK_BLOCK_NUM;
...
std::shared_ptr<USB::DiskInfo> BlueScsiBridge::GetDiskInfo(int target_id)
{
    for (auto cur_disk : USB::DiskInfoList)
    {
